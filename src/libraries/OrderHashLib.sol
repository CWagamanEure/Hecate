...
library OrderHashLib {
    ...
    function batchIdOf(
        ...
        OT.PairId pairId,
        uint64 index
    ) internal view returns (OT.BatchId) {
        return
            OT.BatchId.wrap(
                keccak256(
                    abi.encode(domainSeparator, OT.PairId.unwrap(pairId), index)
                )
            );
    }
    ...
    ) external pure returns (OT.CommitId) {
        OT.CommitId.wrap(
            keccak256(
                abi.encode(trader, OT.BatchId.unwrap(batchId), commitmentHash)
            )
        );
    }
}
